...
import {View, Dimensions, SafeAreaView, FlatList, StyleSheet, Platform} from 'react-native';
...
export class LanguageFilter extends Component {
  ...
  selectUnselectLanguage(name) {
    const {selectedLanguages, updateLanguages} = this.props;
    if (selectedLanguages.includes(name)) {
      let newLanguageList = selectedLanguages.filter((val) => val !== name);
      updateLanguages(newLanguageList);
    } else {
      updateLanguages([...selectedLanguages, name]);
    }
    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={{padding: 10}}>
          ...
          />
          <View>
            <Button title="Any" isActive={selectedLanguages.length === 0} onPress={() => updateLanguages([])} />
            <Button
              title="Your language (English)"
              isActive={selectedLanguages.findIndex((i) => i === 'English') >= 0}
...
const mapDispatchToProps = (dispatch) => {
  return {
    updateLanguages: (data) => dispatch(updateLanguages(data)),
  };
};
